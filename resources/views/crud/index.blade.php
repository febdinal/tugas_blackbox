        ...
        <table class="min-w-full table-auto border">  
            <thead class="border-b">  
                <tr> 
                    ...
                    <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-center">Create Date</th>  
            ...
            <tbody>            
                ...
                <tr class="border-b">  
                    <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{ $post->name }}</td>  
                    <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-center"><b>{{ $post->qty }}</b></td>  
                    <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-center">{!! $post->description !!}</td>  
                    <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-center">{{ $post->created_at->format('Y-m-d') }}</td>  
                    <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-center">  

                        <form onsubmit="return confirm('Apakah Anda Yakin ?');"  
                              action="{{ route('crud.destroy', $post->id) }}" method="POST">  
                            @csrf  
                            @method('DELETE')  
                            <a href="{{ route('crud.edit', $post->id) }}" id="{{ $post->id }}-edit-btn"  
